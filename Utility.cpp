...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    else if( auto* pd = dynamic_cast<Paladin*>(character) )
    {
        //same with paladins
        pd->boostArmor( item.getBoost() * 1.3 );
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
        ds->boostArmor( item.getBoost() * 1.5 );
    }
    else if( auto* dg = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    ...
    {
        pd->boostHitPoints(item->getBoost() * 1.5);
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
...
void useAttackItem(Character* character, Item* item)
{
    ...
    {
        pd->boostAttackDamage(item->getBoost() * 1.33);
    }
    else if( auto* ds = dynamic_cast<DragonSlayer*>(character))
    {
        ...
        character->boostAttackDamage( item->getBoost() * character->getAttackDamage() );
    }
    else if( auto* dg = dynamic_cast<Dragon*>(character) )
    {
